...
class CommandHandler(commands.Cog):
    ...
    async def status(self, ctx: commands.Context):
        ...
        ret_msg += f"> This guild name: {ctx.guild.name}\n"
        ret_msg += f"> This guild id: {ctx.guild.id}\n"
        ret_msg += f"> This channel name: {ctx.channel.name}\n"
        ret_msg += f"> This channel id: {ctx.channel.id}\n"
...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        age_protected_error = "\x1b[0;31mERROR:\x1b[0m Sign in to confirm your age\nThis video may be inappropriate for some users."  # pylint: disable=line-too-long
